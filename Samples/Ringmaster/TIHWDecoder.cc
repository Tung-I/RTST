...
TIHWDecoder::TIHWDecoder(const uint16_t display_width,
    ...
    lazy_level_(), output_fd_(), decoder_epoch_(std::chrono::steady_clock::now())
{
  ...
  if (lazy_level <= DECODE_ONLY) {
    worker_ = thread(&TIHWDecoder::worker_main, this);  // thread(a pointer to member, the object, argument)
...
void TIHWDecoder::add_datagram(const FrameDatagram & datagram)
{
...
void TIHWDecoder::add_datagram(FrameDatagram && datagram)
{
  if (not add_datagram_common(datagram)) {
    return;
  }
...
bool TIHWDecoder::next_frame_complete()
{
  {
...
void TIHWDecoder::consume_next_frame()
{
...
void TIHWDecoder::advance_next_frame(const unsigned int n)
{
  next_frame_ += n;
  clean_up_to(next_frame_);
}

void TIHWDecoder::clean_up_to(const uint32_t frontier)
{
