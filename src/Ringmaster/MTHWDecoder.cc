...
MTHWDecoder::MTHWDecoder(const uint16_t display_width,
    ...
    lazy_level_(), output_fd_(), decoder_epoch_(std::chrono::steady_clock::now())
{
  ...
  if (lazy_level <= DECODE_ONLY) {
    worker_ = thread(&MT
  HWDecoder::worker_main, this);  // thread(a pointer to member, the object, argument)
...
bool MTHWDecoder::add_datagram_common(const FrameDatagram & datagram)
{
...
void MTHWDecoder::add_datagram(const FrameDatagram & datagram)
{
...
void MTHWDecoder::add_datagram(FrameDatagram && datagram)
{
  if (not add_datagram_common(datagram)) {
    return;
  }
...
bool MTHWDecoder::next_frame_complete()
{
  {
...
void MTHWDecoder::consume_next_frame()
{
...
void MTHWDecoder::advance_next_frame(const unsigned int n)
{
  next_frame_ += n;
  clean_up_to(next_frame_);
}

void MTHWDecoder::clean_up_to(const uint32_t frontier)
{
...
// double MTHWDecoder::decode_frame(const Frame & frame)
// {
//   if (not frame.complete()) {
...
// void MTHWDecoder::display_decoded_frame(VideoDisplay & display)
// {
